...
if (getItemViewType(position) == TYPE_SEND){
    ...
    ((SendMessViewHolder) holder).txttime.setText(chatMessageList.get(position).datatime);
}else {
    ((ReceivedViewHolder) holder).txtmess.setText(chatMessageList.get(position).mess);
    ((ReceivedViewHolder) holder).txttime.setText(chatMessageList.get(position).datatime);
}
    }
